...
function _passesAllFilters(cc){
  ...
  if(view==='multicanaux'){let caHors=0,caMag=0;const h=_S.ventesLocalHorsMag?.get(cc);const m2=_S.ventesLocalMag12MG?.get(cc);if(h)for(const d of h.values())caHors+=d.sumCA||0;if(m2)for(const d of m2.values())caMag+=d.sumCA||0;if(caHors<=caMag)return false;}
  ...
  function renderCanalAgence(){
    ...
    if(_filterActive){
      ...
      for(const[cc,info] of _S.chalandiseData.entries()){
        ...
        _nbF++;
        const _mag=_S.ventesLocalMag12MG?.get(cc);
  ...
  function computePhantomArticles(){
    ...
    // soldAtPDV = articles vendus en MAGASIN uniquement (ventesLocalMag12MG = pleine période)
    // Un article vendu une seule fois en 12MG ne doit pas disparaître du radar phantom
    const soldAtPDV=new Set();
    for(const[,artMap]of (_S.ventesLocalMag12MG||new Map()).entries()){for(const code of artMap.keys())soldAtPDV.add(code);}
