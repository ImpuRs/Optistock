...
<div class="periode-section">
  ...
  function _buildTerrOmniBlock(){
    ...
    if(hasOmni){
      ...
      const _totalTip=`${total} clients analysés = union MAGASIN (${_S.ventesClientArticle.size}) + hors-agence, après filtres actifs. Clients exclusivement MAGASIN (sans hors-agence) : ${_nExclMag}. Aucun seuil CA minimum — diffère du tableau Top PDV (seuil 100 €).`;
      const _segTips={'Mono PDV':`CA hors-agence ≤ 100 € OU PDV largement dominant — clients ancrés comptoir. Inclut des clients techniquement présents dans les deux canaux mais avec hors-agence marginal. Pas de seuil CA minimum (${nMono} sur ${total} analysés).`,'Hybrides':`CA PDV > 0 ET CA hors-agence > 100 € et non dominant (caHors ≤ caPDV × 1,5). Acheteurs mixtes PDV + digital.`,'Digital':`CA hors-agence > 100 € et dominant (caHors > caPDV × 1,5 ou CA PDV < 50 €). Acheteurs principalement en ligne ou via rep.`,'Dormants':`Silence > 180 jours ET CA hors-agence < 100 €. Clients inactifs sur tous les canaux.`};
