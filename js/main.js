...
<div class="periode-section">
  ...
  function generateRegionReportText(){
    ...
    L.push(`Utilise TOUTES les données fournies — chaque section doit être couverte.`);
    L.push(`Structure :`);
    L.push(`  1. BILAN — CA, rang, marge, ventilation canaux (1 ligne par canal)`);
    L.push(`  2. STOCK — valeur, dispo, ruptures, dormants`);
    L.push(`  3. RÉSEAU — position vs médiane, familles fortes et en retrait (toutes)`);
    L.push(`  4. CLIENTS — silencieux, captation, nomades`);
    L.push(`  5. MES 3 ACTIONS — engagements concrets avec objectifs chiffrés`);
    L.push(`Ton : direct, factuel, première personne. Pas de prose littéraire, pas de roman.`);
    L.push('');
    L.push(`═══════════════════════════════════════════════════`);
    L.push(`DONNÉES AGENCE ${agence} — ${periodLabel}`);
    if(filtersLabel)L.push(`FILTRES ACTIFS : ${filtersLabel}`);
